...
test("outbound SIP attempt serializes leg-end cleanup with startup and suppresses dialog creation before INVITE", async () => {
  ...
  assert.deepStrictEqual(leg.signalingDetails.allowedAudioCodecs, [SIP_AUDIO_CODEC_ALAW]);
  assert.deepStrictEqual(leg.signalingDetails.allowedDtmfMethods, [SIP_DTMF_METHOD_INFO]);
});
...
test("extension outbound target reuses the extensions listener socket", async () => {
  ...
  assert.deepStrictEqual(resolved.codecs, [SIP_AUDIO_CODEC_ALAW]);
  assert.deepStrictEqual(resolved.dtmfMethods, [SIP_DTMF_METHOD_INFO]);
});
